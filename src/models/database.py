...
class Database:
    ...
    # В database.py, обнови метод add_snippet
    def add_snippet(self, title: str, language: str, cells: list, tags: str = "") -> int:
        """Add a new snippet with multi-cell content."""
        print(f"DEBUG: add_snippet вызван с title={title}, language={language}")  # Отладка
        print(f"DEBUG: cells: {cells}")  # Отладка

        try:
            # Convert cells list to YAML
            import yaml
            yaml_content = yaml.dump(cells, allow_unicode=True)
            print(f"DEBUG: YAML content: {yaml_content}")  # Отладка
            ...
            print(f"DEBUG: Сниппет успешно добавлен, ID: {result}")  # Отладка
            return result
        except Exception as e:
            print(f"DEBUG: Ошибка в add_snippet: {e}")  # Отладка
            raise
